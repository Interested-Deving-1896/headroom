...
**Air-gapped deployments:** `HEADROOM_OFFLINE=1` blocks the outbound traffic the proxy itself initiates: the anonymous usage beacon (which is **on by default**), the update check, the license/usage reporter, HuggingFace model and tokenizer downloads (Python and Rust), the remote Kompress endpoint, and **OTLP metric and Langfuse trace export**. Requests you send *through* the proxy are still forwarded to the upstream you configured — that is your traffic, not Headroom's, and an air-gapped deployment points it at an on-prem endpoint.

It is not yet a whole-process egress kill switch. These paths still reach the network with the flag set, and each needs its own guard: GitHub Copilot device-flow auth (`headroom auth`), the Anthropic/Codex/Copilot subscription pollers, the `headroom install` binary and codebase-memory-mcp downloads, the BFCL eval-dataset fetch, the Headroom Cloud compression modes in the ASGI and LiteLLM integrations, the mem0 backend's OpenAI embedder, and the Python `fastembed` relevance model download. They are enumerated with reasons in `_EGRESS_ALLOWLIST` in `tests/test_offline_egress_chokepoint.py`, which fails if a new unguarded path appears. If you need a hard guarantee today, enforce it at the network layer as well.
