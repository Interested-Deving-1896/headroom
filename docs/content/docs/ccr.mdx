...
Retrieval events in `proxy.log` record the hash, tool, strategy and sizes — not
the retrieved content. To debug with the content itself, opt in:

```bash
HEADROOM_LOG_PAYLOAD_PREVIEW=1 headroom proxy
```

Each retrieval then logs up to 4,096 characters of the original, with common
credentials redacted (key/value secrets, `Authorization` headers, URL passwords,
private keys, JWTs, and GitHub, AWS, Slack, Google, Stripe and OpenAI/Anthropic
keys). Redaction only knows those shapes — customer data and source code pass
through — so leave previews off in any log you share. The same setting governs
the MCP server's tool-call logs. The proxy states which mode it is in when its
log starts.
