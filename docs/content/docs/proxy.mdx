...
| `--offline` / `HEADROOM_OFFLINE` | `false` | Air-gap mode: block the egress the proxy initiates (telemetry, update checks, license reporting, model/tokenizer downloads, remote Kompress, OTLP/Langfuse export). Forwarding your own requests upstream is unaffected. Some install-time and opt-in-cloud paths are not covered yet — see the air-gap notes in `docs/metrics-technical-guide.md`. |
