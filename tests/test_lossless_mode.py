...
def test_router_apply_accepts_lossless_search_byte_measured() -> None:
    """Regression: the acceptance gate in router.apply() measured WORD count, so
    a lossless search fold — which saves BYTES by collapsing a repeated path
    prefix while word count stays flat or *rises* (the heading adds a word) — was
    wrongly discarded as ratio_too_high. The gate now byte-measures lossless
    results, so the free, byte-recoverable win is applied. (compress()/
    ...
    assert len(out) < len(grep)  # accepted (byte-smaller), not ratio_too_high
